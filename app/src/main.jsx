...
var CategoryListItem = React.createClass({ 
  ...
  render: function() {
    return <div>Category: { this.context.router.getCurrentParams().categoryId }</div>;
  },
...
var EventListItem = React.createClass({
  ...
  render: function() {
    if (this.context.router.getCurrentParams().eventId) {
      return <div>event id: {this.context.router.getCurrentParams().eventId}</div>
    } else {
      return (
        <div key="{this.props.event.id}">
        { this.props.event.id } -  <a href={"/#/events/"+this.props.event.id}>{ this.props.event.event }</a> - <a href={"/#/categories/"+this.props.event.channel}>{ this.props.event.channel }</a>
        </div>
      ) 
    }         
  },
...
var routes = (
  <Route handler={App} path="/">
    <Route name="categoryItem" path="categories/:categoryId" handler={CategoryListItem} /> 
    <Route name="categories" path="categories" handler={CategoryList} /> 
    <Route name="eventItem" path="events" handler={EventList} />
    <Route name="event" path="events/:eventId" handler={EventListItem} />
